...
public class ZipUtils {
     ...
     * 压缩文件
     *
     * @param resFilePath 待压缩文件路径:可以是文件或目录
     * @param zipFilePath 压缩文件路径 ,目标文件路径，zip文件
     * @return {@code true}: 压缩成功<br>
     * {@code false}: 压缩失败
     * @throws IOException IO错误时抛出
     */
     ...
     * 解压文件
     *
     * @param zipFilePath 待解压文件路径 .zip file 文件路径
     * @param unzipToDir  解压目标路径
     * @return {@code true}: 解压成功<br>
     * {@code false}: 解压失败
     * @throws IOException IO错误时抛出
     */
    public static boolean unzipFile(String zipFilePath, String unzipToDir) throws IOException {
        return unzipFile(FileUtils.getFileByPath(zipFilePath), FileUtils.getFileByPath(unzipToDir));
    }
