...
import cn.damei.scm.shiro.JedisManager;
import cn.damei.scm.shiro.SerializeObjectUtil;
...
public class JedisShiroSessionToStringRepository implements ShiroSessionRepository {
    ...
    public Session getSession(final Serializable id) {
        ...
        try {
            session = getJedisManager().execute(new JedisTemplate.JedisAction<Session>() {
                @Override
