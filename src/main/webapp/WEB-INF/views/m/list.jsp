...
<body>
    ...
    <div v-show="isTop">
		...
		<div class="filter-box bar-fixed-top">
				<nav flex="box:mean cross:center" class="bar bar-filter">
					...
					<a href="javascript:;" class="bar-item " @click="show('brand')" :class="[brandshow?'_active':'']">
						品牌
					...
					<!--注意： 升序 请加class「_asc」-->
					<a flex="cross:center main:center" href="javascript:;" class="bar-item item-sort " @click="show('sort')" :class="{'_asc':sortShow,'_active':sortShow}">
						价格
						<svg class="svg-sort ">
				...
				<!--/.filter-content-->
				<div class="filter-content " :class="[brandshow?'':'hidden']">
					<div class="filter-letter-box">	
						<!--
							注意：svg-arrow的箭头向上 class「_up」，箭头向下 class 「_down」时，filter-letter加class「_show」 显示全部
						-->
						<svg class="svg-arrow _up"><use xlink:href="#svg-arrow"/></svg>
						<div flex=" " class="filter filter-letter _show" >
							<span class="item" v-for="item in brandLetter" :class="[item.active?'_active':'']" @click="chooseletter($index,item.letter)">{{item.letter}}</span>
						</div>
						<!--/.filter-letter-->
					</div>
					<!--/.filter-letter-box-->
					<div class="filter-brand-box">
						<div flex=" " class="filter filter-brand">
							<div class="item-fiter _ellipsis" v-for="item in brand":class="[item.active?'_active':'']" @click="chooseBrand($index,item.id)"> {{item.name}}</div>
						</div>
						<!--/.filter-brand-->
					</div>
					<!--/.filter-brand-boxs-->
		...
		<div class="content" > 
            ...
            <div class="modal-overlay " :class="[brandshow?'_in':'']" @click="brandshow=false"></div>
                ...
                <aside class="aside aside-1">
                    ...
                    <div class="list-block item-goods " :class="[item.active?'_active':'']" v-for="item in productListLeft">
                        <a href="/product/{{item.id}}/m" flex="main:center" class="item-media" @click="go">
                            <img v-lazyload="item.loadableMainImgUrl" src="/static/m/images/material/p-l-334x340.jpg" alt="商品图片" >
                        </a>
    ...
    <script src="${ctx}/static/m/js/list.js?v=ea38b0f7"></script>
</body>
</html>