  ...
  var WebUploaderComponent = Vue.extend({
    ...
    props: {
     ...
     objectId: {
      ...
      wServer: {
        type: String,
        default: ctx + '/api/repository/Product/importProduct'
      },
    ...
    data: function() {
      return {
        config: {
          swf: ctx + '/static/admin/vendor/webupload/Uploader.swf'
        },
        uploader: null,
    ...
    ready: function() {
      ...
      try {
        ...
        this.uploader.on('uploadSuccess', function(file, response) {
          if(self.type == 'forSku'){
            //仅仅是为了动态生成新增商品专用的
            self.$dispatch('webupload-upload-success-' + self.type, self.ind, response, self.objectId);
          }else{
            self.$dispatch('webupload-upload-success-' + self.type, file, response, self.objectId,self.ind);
          }
          
        });
        this.uploader.on('uploadError', function(file, reason) {
          self.$dispatch('webupload-upload-error-' + self.type, file, response, self.objectId);
        });
