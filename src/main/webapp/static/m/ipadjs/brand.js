...
(function(window, document, $) {
    ...
    function Brand(selector, template) {
        ...
        // this.$clearSearchText = $('[data-brand="clearsearchtext"]'); // 清除搜索内容
        this.fly = $('#_fly'); // 飞心
    ...
    Brand.prototype.dynamicSearch = function(val) {
        ...
        $.ajax({
            ...
            dataType:'json',
            beforeSend:function() {
                self.searchHistory.addClass('disN').removeClass('disB');
            },
            success:function(res) {
                if(res.code == 1) {
                    ...
                    // 显示出来
                    self.searchHistory.removeClass('disN').addClass('disB');
                }
            }
        })
    }
    ...
    Brand.prototype.event = function() {
        ...
        // 鼠标移出品牌区域则消失
        // self.selector.on('mouseout', '[data-brands="all"]', function(e) {
        //     e.stopPropagation();
        ...
        self.search.on('input', function(e) {
            ...
            timer = setTimeout(function() {
                ...
                if($.trim(val) === '') {
                    self.searchHistory.addClass('disN').removeClass('disB');
                    self.search.removeClass('_focus');
                    return;
                }
                else {
                    self.search.addClass('_focus');
                }
        ...
        self.$goList.on('submit', function(e) {
            window.location.href = ctx + '/ipad/list?keyword=' + self.search.val()
    ...
    // 开始飞
    Brand.prototype.startFly = function(item, callback) {
        var position = item.getBoundingClientRect()
        this.flyTo(window.innerWidth - position.left, position.top, callback)
    }

    // 处理数字飞向心愿
    Brand.prototype.flyTo = function(right, top, callback) {
        var self = this;
        // self.fly.velocity("finish");
        self.fly.velocity("stop");
        self.fly.css('right', right).css('top', top);
        self.fly.velocity({
            right:30,
            top: 30
        },{
    ...
    Brand.prototype.imgLazy = function(item) {
        ...
        item.find('img').lazyload({
            placeholder : ctx + "/static/m/images/material/p-l-334x340.jpg",
            effect : "fadeIn",
            threshold : 200
        });
    }
